...
// ---------------------------------------------------------------------------
// Detection layer: shared state + runtime discovery (leaf — no cross-layer calls)
// ---------------------------------------------------------------------------

class RuntimeManagerDetection {
  protected runtimes = new Map<string, RuntimeInfo>();
  protected initialized = false;
  protected config: Map<string, RuntimeConfig> = new Map();
...
// ---------------------------------------------------------------------------
// Version layer: semantic-version comparison and compatibility reporting
// ---------------------------------------------------------------------------

class RuntimeManagerVersion extends RuntimeManagerDetection {
  /**
   * Check version compatibility with detailed reporting
   */
  protected checkVersionCompatibility(runtime: RuntimeInfo, config?: RuntimeConfig): boolean {
   ...
   */
  protected getRuntimeInstallationSuggestions(language: string): string[] {
...
// ---------------------------------------------------------------------------
// Access layer: runtime lookup, fallback resolution, and statistics
// ---------------------------------------------------------------------------

class RuntimeManagerAccess extends RuntimeManagerVersion {
  /**
...
// ---------------------------------------------------------------------------
// Spawning layer: analyzer process orchestration (public top layer)
// ---------------------------------------------------------------------------

/**
 * Runtime manager.
 */
export class RuntimeManager extends RuntimeManagerAccess {
  /**
...
class TypeScriptAnalyzer implements LanguageAnalyzer {
  ...
  async analyze(files: string[], options?: any): Promise<AnalysisResult> {
    ...
    try {
      ...
      return {
        violations: extractViolations(auditResult),
        indexEntries: collectIndexEntries(auditResult.metadata),
        metrics: {
          filesAnalyzed: files.length,
          executionTime
        },
        errors: extractErrors(auditResult)
      };
    } catch (error) {
      const executionTime = Date.now() - startTime;
...
/** Build a single function index entry from a collected-function record. */
function toFunctionIndexEntry(func: any): any {
  return {
...
/** Extract function index entries from audit metadata (dedup across both maps). */
function collectIndexEntries(metadata: any): any[] {
  const indexEntries: any[] = [];

  // Try to get functions from the audit metadata if available
  if (metadata?.collectedFunctions) {
    for (const func of metadata.collectedFunctions) {
      indexEntries.push(toFunctionIndexEntry(func));
    }
  }

  // Add component information if available
  if (metadata?.fileToFunctionsMap) {
    for (const funcs of Object.values(metadata.fileToFunctionsMap)) {
...
/** Flatten violations from all analyzer results. */
function extractViolations(auditResult: any): any[] {
  const violations: any[] = [];
  for (const analyzerResult of Object.values(auditResult.analyzerResults ?? {}) as any[]) {
    violations.push(...analyzerResult.violations);
  }
  return violations;
}

/** Flatten errors from all analyzer results. */
function extractErrors(auditResult: any): any[] {
  const errors: any[] = [];
  for (const analyzerResult of Object.values(auditResult.analyzerResults ?? {}) as any[]) {

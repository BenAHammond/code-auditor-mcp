 ...
 * package and used by the installer). The repo-root skills/code-auditor/ copy
 * is a build artifact generated by `build:skills`. These tests verify the root
 * copy is byte-identical to the canonical source for every file in the skill
 * directory.
 */
describe('Skill deduplication — root copy matches canonical', () => {
  const CANONICAL_DIR = resolve(__dirname, '..', 'plugin', 'skills', 'code-auditor');
  const ROOT_DIR = resolve(__dirname, '..', '..', 'skills', 'code-auditor');

  // Build the expected file list from the canonical directory
  const { readdirSync, readFileSync, existsSync } = require('fs');
  const canonicalFiles = readdirSync(CANONICAL_DIR).filter(
    (f: string) => f.endsWith('.md'),
  );

  it('root skills directory exists', () => {
    expect(existsSync(ROOT_DIR)).toBe(true);
  });

  it('root SKILL.md is byte-identical to canonical', () => {
    const canonical = readFileSync(resolve(CANONICAL_DIR, 'SKILL.md'));
    const root = readFileSync(resolve(ROOT_DIR, 'SKILL.md'));
    expect(Buffer.compare(canonical, root)).toBe(0);
  });

  it('root SKILL-RULE-KINDS.md is byte-identical to canonical', () => {
    const canonical = readFileSync(resolve(CANONICAL_DIR, 'SKILL-RULE-KINDS.md'));
    const root = readFileSync(resolve(ROOT_DIR, 'SKILL-RULE-KINDS.md'));
    expect(Buffer.compare(canonical, root)).toBe(0);
  });

  it('root SKILL-SEARCH.md is byte-identical to canonical', () => {
    const canonical = readFileSync(resolve(CANONICAL_DIR, 'SKILL-SEARCH.md'));
    const root = readFileSync(resolve(ROOT_DIR, 'SKILL-SEARCH.md'));
    expect(Buffer.compare(canonical, root)).toBe(0);
  });

  it('root copy has no extra .md files beyond canonical', () => {
    const rootFiles = readdirSync(ROOT_DIR).filter((f: string) => f.endsWith('.md'));
    const canonicalSet = new Set(canonicalFiles);
    const extras = rootFiles.filter((f: string) => !canonicalSet.has(f));
    expect(extras).toEqual([]);
  });

  it('root copy has the same .md files as canonical (no missing files)', () => {
    const rootFiles = new Set(
      readdirSync(ROOT_DIR).filter((f: string) => f.endsWith('.md')),
    );
    const missing = canonicalFiles.filter((f: string) => !rootFiles.has(f));

...
describe('Rule Registry', () => {
  ...
  it('has entries for every known analyzer', () => {
    ...
    // (not a fixed set), and its former `config-error`/`engine-error` entries
    // moved to the diagnostics channel (CoverageDiagnostic.kind) — see the NOTE
    // at the top of ruleRegistry.ts.
    for (const name of [
      'solid', 'dry', 'data-access', 'react', 'documentation',
      'schema', 'styles', 'conventions', 'cross-domain',
    ]) {

...
describe('rankFilesByPriority', () => {
  it('ranks a file with a critical finding above one with only warnings', () => {
    const ranked = rankFilesByPriority([
      v({ file: 'b.ts', severity: 'warning', line: 1 }),
  ...
  it('breaks severity ties by total finding count (descending)', () => {
    const ranked = rankFilesByPriority([
      v({ file: 'few.ts', severity: 'warning', line: 1 }),
      v({ file: 'many.ts', severity: 'warning', line: 1 }),
      v({ file: 'many.ts', severity: 'warning', line: 2 }),
      v({ file: 'many.ts', severity: 'warning', line: 3 }),
    ]);
  ...
  it('breaks count ties by deterministic path order', () => {
    const ranked = rankFilesByPriority([
      v({ file: 'z.ts', severity: 'warning', line: 1 }),
      v({ file: 'a.ts', severity: 'warning', line: 1 }),
    ]);
  ...
  it('reports the max severity and keeps every violation grouped per file', () => {
    const ranked = rankFilesByPriority([
      v({ file: 'mixed.ts', severity: 'suggestion', line: 1 }),
...
describe('orderFindingsWithinFile', () => {
  it('orders critical → warning → suggestion and preserves input order for ties', () => {
    const ordered = orderFindingsWithinFile([
      v({ file: 'f.ts', severity: 'suggestion', line: 1 }),
      v({ file: 'f.ts', severity: 'critical', line: 2 }),
      v({ file: 'f.ts', severity: 'warning', line: 3 }),
      v({ file: 'f.ts', severity: 'critical', line: 4 }),
    ]);
    expect(ordered.map((x) => x.severity)).toEqual(['critical', 'critical', 'warning', 'suggestion']);
  });

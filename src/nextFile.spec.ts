...
describe('rankFilesByPriority', () => {
  ...
  it('throws on an unknown severity rather than silently mis-ranking it', () => {
    // A stale `advisory` (pre-`high` rename) must not sort to `undefined` and
    // vanish — the rank lookup is the last line of defense after the snapshot
    // version gate (SNAPSHOT_VERSION) catches known renames.
    expect(() =>
      rankFilesByPriority([v({ file: 'f.ts', severity: 'advisory' as Violation['severity'], line: 1 })]),
    ).toThrow(/Unknown severity "advisory"/);
  });
});
